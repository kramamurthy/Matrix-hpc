...
//Workstealing functions
	...
	case 15: {
       ...
       //Package operation to get the information of all idle cores.
	case 16: {
				int32_t load;
				if (package.virtualpath().empty()) {
					operation_status = -1;
					...
					//load = ready_queue->get_length() - worker.num_idle_cores;	// Get the current load
					load = worker->get_idle_core_info();
					//load = rqueue.size() - worker->num_idle_cores;     // Get the current load
					//cout << "loadinfo: worker_id = " << worker->selfIndex << " rqueue = " << rqueue.size() << " mqueue = " << mqueue.size() << " load = " << load << endl;
				}
	...
	//Package operation to insert the HPC task on the specified node.
	case 17:
	{
		int32_t result;
		cout << "Method to transfer HPC task" << endl;
		if (package.virtualpath().empty()) {
			cout << "Package is empty " << endl;
			operation_status = -1;
		} 
		else
		{
			worker->Receive_HPC_Task(package);
		}
		...
		if (TCP == true) 
		{
			if(LOGGING)
			{
				log_fp << "Task Received status = " << result << endl;
			}
			r = send(client_sock, &result, sizeof(int32_t), 0);
		}
		else 
		{
			r = sendto(client_sock, &result, sizeof(int32_t), 0,
			(struct sockaddr *) &fromAddr, sizeof(struct sockaddr));
		}
		//msg_count[4]++;
		if (r <= 0) 
		{
			cout<< "HPC insert information: Server could not insert HPC task in the ready queue of client r = " << r << endl;
		}
	}break;
	//Release resources if enough cores are not obtained to run a HPC task
	case 18:
	{
		int32_t result;
		if (package.virtualpath().empty())
		...
		{
			cout << "CALLING RELEASE RESOURCE" << endl;
 ...
 *                 }*/
	...
	case 24: {
		int load;
		load = worker->get_complete_queue_info();
		if (TCP == true) {
                        if(LOGGING) {
                                log_fp << "MONITORING INFORMATION = " << load << endl;
                        }
                ...
                msg_count[6]++;
                if (r <= 0) {
                        cout << "Monitoring Info: Server could not send monitoring information to client: sendto r = " << r << endl;
                }
