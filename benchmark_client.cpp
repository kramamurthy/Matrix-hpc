...
int main(int argc, char* argv[]) {
	...
	//double currentmillis = getTime_usec();
        //cout << "THE CLIENT STARTED AT " << currentmillis << endl;
	if(argc != 14) {
        	cout << "Usage: " << argv[0] << "\tper_client_task\tneighborlist_file\tzht_config_file\tprotocol[TCP/UDP]\tsleep_time\tsubmissionmode[0/1/2]\tmax_tasks_per_package\tindex\tnum_tasks\tprefix\tshared\tDAG_choice\tNum_Of_Cores" << endl;
                exit(1);
        }
...
//cout << cmd << endl;
	...
	int num_of_cores=atoi(argv[13]);
	cout << "The number of cores required by each task is " << num_of_cores << endl;
	pthread_t monitor_thread;
	if(index == 1) {
		monitor_thread = mc.monitor(num_tasks, testClient, num_of_cores);
	}
	//int num_of_cores=atoi(argv[13]);
	mc.initializeTasks(per_client_task, sleep_time, mode, max_tasks_per_package, testClient, DAG_choice, num_of_cores);
	//mc.submitTasks(mode, testClient);
	if(index == 1){
